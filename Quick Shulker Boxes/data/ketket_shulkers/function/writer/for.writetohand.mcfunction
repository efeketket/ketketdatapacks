...
$execute if data entity @e[tag=this,type=hopper_minecart,limit=1] Items[0].components."minecraft:container"[{slot:$(for)}].item{id:"minecraft:knowledge_book",count:1} run data remove entity @e[tag=this,type=hopper_minecart,limit=1] Items[0].components."minecraft:container"[{slot:$(for)}].item
