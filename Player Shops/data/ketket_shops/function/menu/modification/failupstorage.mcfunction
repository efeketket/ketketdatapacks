...
execute as @a[tag=menuopener,distance=..8] if score @s unique_shopid = @e[type=interaction,limit=1,sort=nearest,tag=main] unique_shopid run tellraw @p[tag=menuopener,sort=nearest,limit=1] ["","[",{text:"Player Shop",color:"green"},"]"," You reached storage limit."]