...
execute if score @s totalearn matches ..0 if score @s currentstock matches ..0 run data modify entity @e[type=item_display,sort=nearest,tag=shopdata,limit=1] item.components."minecraft:custom_data" set value {Product:[{}],Price:[{}],StorageCheck:[{}],ValueHolder:[{}],PriceDisp:[{}],ProductDisp:[{}],SellingHolder:[{}]}